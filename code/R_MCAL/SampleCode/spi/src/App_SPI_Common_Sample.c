 ...
 * 2.0.0:  17/07/2023  : Updated SW-VERSION. Re-write test app to support SPI Multi-core
 * 1.5.0:  29/03/2023  : Move Spi_Init() invoked before Port_Init()
 *         07/03/2023  : Updated SW-VERSION
 * 1.4.3:  18/05/2022  : Add API Spi_SyncTransmit
 * 1.4.2:  02/03/2022  : Updated SW-VERSION
 * 1.2.0:  28/08/2020  : Release
 * 1.1.0:  25/06/2020  : Release
 * 1.0.5:  25/06/2020  : Update single instance sequence for master/slave
 *                       communication
 * 1.0.4:  05/02/2020  : Make common sample app of all X2x SPI HW IPs.
 * 1.0.3:  09/08/2019  : Update for using typical APIs of SPI Driver.
 * 1.0.2:  23/11/2018  : Update argument name of Spi_Init().
 * 1.0.1:  30/03/2017  : Remove unnecessary items
...
#ifdef CCRH
uint32 App_GaaTransmitData0[] =
#else
STATIC Spi_DataBufferType App_GaaTransmitData0[SPI_DATA_BYTES] =
#endif
{
  11, 12, 13, 14
};
...
#ifdef CCRH
uint32 App_GaaReceiveData0[SPI_DATA_BYTES] = { 0 };
#else
STATIC Spi_DataBufferType App_GaaReceiveData0[SPI_DATA_BYTES] = { 0 };
#endif

/* Transmit data for SpiChannel1 */
#ifdef CCRH
uint32 App_GaaTransmitData1[] =
#else
STATIC Spi_DataBufferType App_GaaTransmitData1[SPI_DATA_BYTES] =
#endif
{
  21, 22, 23, 24
};
...
#ifdef CCRH
uint32 App_GaaReceiveData1[SPI_DATA_BYTES] = { 0 };
#else
STATIC Spi_DataBufferType App_GaaReceiveData1[SPI_DATA_BYTES] = { 0 };
#endif
...
***********************************************************************************************************************/
  ...
  SPI_RESULT_ASSERT(E_OK == Spi_WriteIB(Spi_SpiChannel1, (Spi_DataBufferType *)App_GaaTransmitData1));  
  #endif

  #if ((SPI_CHANNEL_BUFFERS_ALLOWED == SPI_EB) || (SPI_CHANNEL_BUFFERS_ALLOWED == SPI_IBEB))
  SPI_RESULT_ASSERT(E_OK == Spi_SetupEB(Spi_SpiChannel0, (Spi_DataBufferType *)App_GaaTransmitData0, (Spi_DataBufferType *)App_GaaReceiveData0, 2));
  #endif
  ...
  SPI_RESULT_ASSERT(E_OK == Spi_AsyncTransmit(Spi_SpiSequence1));
  #endif
  /* This service is for transmitting in master unit */
  #if ((SPI_LEVEL_DELIVERED == SPI_LEVEL_0) || (SPI_LEVEL_DELIVERED == SPI_LEVEL_2))
  SPI_RESULT_ASSERT(E_OK == Spi_SyncTransmit(Spi_SpiSequence0));
  #endif
  ...
  #if ((SPI_CHANNEL_BUFFERS_ALLOWED == SPI_IB) || (SPI_CHANNEL_BUFFERS_ALLOWED == SPI_IBEB))
  SPI_RESULT_ASSERT(E_OK == Spi_ReadIB(Spi_SpiChannel1, (Spi_DataBufferType *)App_GaaReceiveData1));
  #endif

  /* Get result of the Jobs */
  SPI_RESULT_ASSERT(SPI_JOB_OK == Spi_GetJobResult(Spi_SpiJob0));
  SPI_RESULT_ASSERT(SPI_JOB_OK == Spi_GetJobResult(Spi_SpiJob1));

  /* Get result of the specified sequences */
  SPI_RESULT_ASSERT(SPI_SEQ_OK == Spi_GetSequenceResult(Spi_SpiSequence0));
  SPI_RESULT_ASSERT(SPI_SEQ_OK == Spi_GetSequenceResult(Spi_SpiSequence1));
  ...
  SPI_RESULT_ASSERT(E_OK == Spi_AppVerifyData((Spi_DataBufferType *)App_GaaTransmitData0, (Spi_DataBufferType *)App_GaaReceiveData1, 2));
  SPI_RESULT_ASSERT(E_OK == Spi_AppVerifyData((Spi_DataBufferType *)App_GaaTransmitData1, (Spi_DataBufferType *)App_GaaReceiveData0, 2));

  /* Clear buffer */
  Spi_ClearBuffer((Spi_DataBufferType *)App_GaaReceiveData1, sizeof(App_GaaReceiveData1));
  Spi_ClearBuffer((Spi_DataBufferType *)App_GaaReceiveData0, sizeof(App_GaaReceiveData0));
...
***********************************************************************************************************************/
  ...
  #if ((SPI_LEVEL_DELIVERED == SPI_LEVEL_1) || (SPI_LEVEL_DELIVERED == SPI_LEVEL_2))
  SPI_RESULT_ASSERT(E_OK == Spi_AsyncTransmit(Spi_SpiSequence0));
  #endif

  /* Cancel on-going sequences */
  #if (SPI_FORCE_CANCEL_API == STD_ON)
  Spi_ForceCancel(Spi_SpiSequence0);
  Spi_ForceCancel(Spi_SpiSequence1);
  #endif
  ...
  /* Get result of the Jobs */
  SPI_RESULT_ASSERT(SPI_JOB_FAILED == Spi_GetJobResult(Spi_SpiJob0));
  SPI_RESULT_ASSERT(SPI_JOB_FAILED == Spi_GetJobResult(Spi_SpiJob1));

  /* Get result of the specified sequences */
  SPI_RESULT_ASSERT(SPI_SEQ_CANCELED == Spi_GetSequenceResult(Spi_SpiSequence0));
  SPI_RESULT_ASSERT(SPI_SEQ_CANCELED == Spi_GetSequenceResult(Spi_SpiSequence1));
...
***********************************************************************************************************************/
  ...
  /* This service is for transmitting the master sequence */
  #if ((SPI_LEVEL_DELIVERED == SPI_LEVEL_1) || (SPI_LEVEL_DELIVERED == SPI_LEVEL_2))
  SPI_RESULT_ASSERT(E_OK == Spi_AsyncTransmit(Spi_SpiSequence2));
  #endif

  /* Cancel on-going sequences */
  #if (SPI_CANCEL_API == STD_ON)
  Spi_Cancel(Spi_SpiSequence1);
  Spi_Cancel(Spi_SpiSequence2);
  #endif
  ...
  /* Get result of the Jobs */
  SPI_RESULT_ASSERT(SPI_JOB_OK == Spi_GetJobResult(Spi_SpiJob0));
  SPI_RESULT_ASSERT(SPI_JOB_QUEUED == Spi_GetJobResult(Spi_SpiJob2));

  /* Get result of the specified sequences */
  SPI_RESULT_ASSERT(SPI_SEQ_OK == Spi_GetSequenceResult(Spi_SpiSequence1));
  SPI_RESULT_ASSERT(SPI_SEQ_CANCELED == Spi_GetSequenceResult(Spi_SpiSequence2));
