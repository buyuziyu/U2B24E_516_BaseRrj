...
/* Module       = App_FEE_U2B6-E_Sample.c                                                                             */
/*====================================================================================================================*/
/*                                                     COPYRIGHT                                                      */
/*====================================================================================================================*/
...
***********************************************************************************************************************/
/*
 * 1.1.1:  30/06/2025:  Add function Guard_Enable_PE
...
void Mcu_Init(void)
{
  ...
  CLKKCPROT1  = FEE_KCPROT_SET;
  HSOSCSTPM   = 0x00000000UL;
  CLKKCPROT1  = FEE_KCPROT_CLR;
...
**                                                  Stub Prepare Areas                                                **
***********************************************************************************************************************/
void Fee_StubPrepareAreas(void)
{
  /* Stub all areas before initalazation Fee_Init */
  uint32 LulMemAddress = FEE_MEM_DATA0_START_ADDRESS;
  do
  {
    GddReturnValue = Mem_59_Renesas_Erase(FEE_SAMPLE_ZERO, LulMemAddress, 0x1000);
    if (E_OK == GddReturnValue)
    {
      while (MEM_JOB_PENDING == Mem_59_Renesas_GetJobResult(FEE_SAMPLE_ZERO))
      {
        Mem_59_Renesas_MainFunction();
      }

      if (FEE_MEM_DATA0_END_ADDRESS < LulMemAddress)
      {
        break;
      }
      else
      {
        LulMemAddress = LulMemAddress + 0x1000;
      }
    }
  } while (E_OK == GddReturnValue);
}
