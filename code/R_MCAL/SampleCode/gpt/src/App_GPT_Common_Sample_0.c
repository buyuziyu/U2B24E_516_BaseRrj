...
/* Module       = App_GPT_Common_Sample_0.c                                   */
/*============================================================================*/
/*                                  COPYRIGHT                                 */
/*============================================================================*/
 ...
 *                       2.Correct process for checking the wakeup notification
 *                       3.Change using from marco GPT_KERNEL_CORE_ID to global
 *                       variable Gpt_GucKernelCoreId
 *  2.0.1  10/01/2024  : Update boundary for wakeup U2B24
 *         08/11/2023  : Update wakeup sequence for U2B24
 *  2.0.0  26/07/2023  : Remove multi-instance, update with new configuration
 *         12/05/2023  : Update SW version 2.0.0
 *  1.3.2  27/01/2022  : Place barrier synchronization after Gpt_Deinit
...
int main(void)
{
    ...
    (GPT_SW_PATCH_VERSION == Lddversioninfo.sw_patch_version))
  {
    ...
    if (E_OK == LddInitCheckValue)
    {
      ...
      while(GaaNotificationEntered_Core0[0] < Notify_Count_Continuous);
      while(GaaNotificationEntered_Core0[1] < Notify_Count_One_Shot);
      ...
      #if (STD_ON == GPT_TIMER_IP_ATU || STD_ON == GPT_TIMER_IP_GTM)
      while(GaaNotificationEntered_Core0[6] < Notify_Count_Continuous);
      ...
      /* Wait until core 1 is done with checking the notification */
      ...
      /* Wait until core 1 main task is done */
