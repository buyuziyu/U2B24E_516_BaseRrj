...
/* Module       = App_CAN_PostBuildVariant_Sample.c                           */
/*============================================================================*/
/*                                  COPYRIGHT                                 */
/*============================================================================*/
 ...
 * 2.1.1: 12/08/2024  : Update Id, checkpoint conditions for CAN XL controller
 *        24/06/2024  : Update to support de-init for CANXL controller
 *        31/05/2024  : Update to support Post build variant for CANXL
 *        21/05/2024  : Change CAN_APPL_CODE to CAN_CALLOUT_CODE
 * 2.0.3: 15/04/2024  : Update to support postbuild variant
 * 2.0.2: 21/03/2024  : Initial Version
 */
/******************************************************************************/
/*******************************************************************************
**                      Include Section                                       **
...
#endif
{
  ...
  into START mode */
  Can_Config = Can_Config_Variant_1;
  Appl_Can_Init();
  #if defined (CAN_U2BxE_USED) && defined (RUN_OTHER_PE)
  BoundIntToPE6();
  #endif
  ...
  /* Sample app support for de-initialization feature at AR431 */
  ...
  Can_Config = Can_Config_Variant_2;
  ...
  #endif /* (!defined CAN_U2C2_USED) && (!defined CAN_U2B6E_USED) */
  ...
  if (GucReturnCount == TOTAL_CHECKPOINT_SINGLECORE_MULTIVARIANT)
  {
  ...
  P2CONST(uint8, CAN_APPL_DATA, AUTOMATIC) CanSduPtr)
{
  ...
  if ((0x068 == CanId) || (0x02 == CanId) || ((0x085 | 0x40000000UL) == CanId))
  {
