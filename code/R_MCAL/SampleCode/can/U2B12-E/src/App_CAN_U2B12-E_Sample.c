...
/* Module       = App_CAN_U2B12-E_Sample.c                                    */
/*============================================================================*/
/*                                  COPYRIGHT                                 */
/*============================================================================*/
...
#define PFCEAE_MASK(n)                        ((n - 1U) & 0x008U) >> 3U
...
CONST(uint32, CAN_APPL_CONST) Can_GaaIntCntlRegsInfo[] =
{
  ...
  /* HW Unit: CAN4 */
  670UL, /* EIC682(INTRCAN2ERR)   */
  671UL, /* EIC683 (INTRCAN2REC)   */
  672UL, /* EIC684 (INTRCAN2TRX)   */
  /* HW Unit: CAN8 */
...
void Mcu_Init(void)
{
  ...
  /* Enable TAUD module */
  MSR_OSTM &=0xFFFFFF80UL;
  CAN_MSR_TAUD = CAN_MSR_ACTIVE;
  while (CAN_MSR_TAUD != CAN_MSR_ACTIVE);
  #if defined(CAN_U2B12E_USED) || defined(CAN_U2B24E_516_USED)
  /* Enable ETN module */
  CAN_MSR_ETN = CAN_MSR_ACTIVE;
  while (CAN_MSR_ETN != CAN_MSR_ACTIVE);
  #endif
...
void Port_Init(void)
{
  ...
  Port_Sub_Init(0, 2, 13, APP_ALT7_OUT, 2, 14, APP_ALT7_IN);
  Port_Sub_Init(8, 10, 6, APP_ALT4_OUT, 10, 5, APP_ALT3_IN);
  
  Port_Sub_Init(0, 2, 8, APP_ALT16_OUT, 1, 7, APP_ALT7_IN);
  Port_Sub_Init(1, 2, 6, APP_ALT15_OUT, 2, 7, APP_ALT4_IN);
                   ...
                   uint8 pin_in, uint8 alt_mode_in)
{
  ...
  LpReg->usPFCEAE = (uint16)((LpReg->usPFCEAE & ~(1 << pin_out))
                                            | (PFCEAE_MASK(alt_mode_out) << pin_out));
  ...
  LpReg->usPFCEAE = (uint16)((LpReg->usPFCEAE & ~(1 << pin_in))
                                            | (PFCEAE_MASK(alt_mode_in) << pin_in));
...
void Gpt_Init(void)
{
  ...
  /*  Enable interrupt, Interval timer mode */
  CAN_OSTM0CTL = 0x80UL;
  *((volatile uint16*)0xfff802d0) = 0x4fU;
  /* enable interrupt for OSTM channel 0, enable reference table jump */
  CAN_ICOSTM018 = 0x0040U;
  CAN_OSTM0TS = 0x01U;
  #else
