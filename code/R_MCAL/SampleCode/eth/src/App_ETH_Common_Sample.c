 ...
 * 2.3.1: 31/03/2025    : Added Logic code for GWCA controller 
 ...
 * 2.1.1: 30/08/2024    : Added Eth_InitT1s to initialize T1s Phy
 *                      : Remove ETH_USING_MACRO and enable/diasable HW_IP macro by STD_ON/STD_OFF
 *                      : Change PHY_TRCV_IDX to 0x1FU
 * 2.0.0: 02/08/2023    : Add return 0 for core max
 *        21/12/2022    : Add macro condition to check versioninfo
 * 1.5.5: 16/12/2022    : Added call Guard_Enable_PE(),Guard_Init() and Updated main_PE1 to main_PE5
 * 1.5.4: 01/08/2022    : Improve preprocessor control for MII and
 *                        Physical Address Filter functions
 * 1.5.3: 24/06/2022    : Fix a bug in implementing the SendData function that
 *                        causes the continuous transmission to stop working
 *                        in interrupt mode.
 * 1.5.1: 12/01/2021    : Update PHY_TRCV_IDX for U2Bx devices
 *        01/12/2021    : Improve the implementation of SendData function
 ...
 * 1.0.4: 16/03/2020    : Updated Eth 2 channel.
 * 1.0.3: 25/08/2019    : Updated to demonstrate Stream Filtering, Address.
 *                        Filtering and gPTP timestamping feature.
 * 1.0.2: 05/08/2019    : Edited sequence and added more APIs to follow MRS.
 * 1.0.1: 14/12/2018    : Corresponds to 2 units.
 * 1.0.0: 15/01/2018    : Initial Version.
 */
/******************************************************************************/
...
volatile uint8 GaaRxSrcAddr[ETH_TOTAL_CTRL_CONFIG][SRC_MACADDR_LEN];
volatile uint8 GucTxConfirmed[ETH_TOTAL_CTRL_CONFIG];
volatile uint16 GusMsgLength[ETH_TOTAL_CTRL_CONFIG];
volatile uint16 GusRxFrameCnt[ETH_TOTAL_CTRL_CONFIG];
volatile uint16 GusRxLenByte[ETH_TOTAL_CTRL_CONFIG];
volatile Eth_FrameType GusRxFrameType[ETH_TOTAL_CTRL_CONFIG];
...
static volatile uint32 GulPortId;
#endif  
...
/* Board MAC address */
static uint8 mac_board_addr[4][SRC_MACADDR_LEN] =
{
  {0x74U, 0x90U, 0x50U, 0x00U, 0x00U, 0x00U},
  {0x74U, 0x90U, 0x50U, 0x00U, 0x00U, 0x01U},
  {0x74U, 0x90U, 0x50U, 0x00U, 0x00U, 0x02U},
  {0x74U, 0x90U, 0x50U, 0x00U, 0x00U, 0x03U}
};

/* Target MAC address */
static uint8 mac_tgt_addr[DST_MACADDR_LEN] = {0x74U, 0x90U, 0x50U, 0xFFU, 0xFFU, 0xFFU};

/* IEEE 1722 frames MAC address*/
static uint8 mac_tgt_addr0[DST_MACADDR_LEN] = {0x91U, 0xE0U, 0xF0U, 0x00U, 0x00U, 0x00U};
static uint8 mac_tgt_addr1[DST_MACADDR_LEN] = {0x91U, 0xE0U, 0xF0U, 0x00U, 0xFEU, 0xFFU};

/* Network control MAC address */
static uint8 mac_nc_addr[DST_MACADDR_LEN] = {0x01U, 0x80U, 0xC2U, 0x00U, 0x00U, 0x0EU};

/* Valid multicast MAC address used for Eth_UpdatePhysAddrFilter */
static uint8 mac_multi_addr0[DST_MACADDR_LEN] =  {0x11U, 0x22U, 0x33U, 0x44U, 0x55U, 0x66U};
/* Invalid multicast MAC address used for Eth_UpdatePhysAddrFilter */
static uint8 mac_multi_addr1[DST_MACADDR_LEN] = {0x11U, 0x22U, 0x33U, 0x44U, 0x55U, 0x77U};

#if (ETH_STREAM_FILTERING == STD_ON)
static uint8 streamId0[STREAMID_LEN] = {0x11U, 0x22U, 0x33U, 0x44U, 0x55U, 0xFFU, 0x00U, 0x00U};
#endif
...
#endif
{
  ...
  for (LucCtrlCnt = 0; LucCtrlCnt < ETH_TOTAL_CTRL_CONFIG; LucCtrlCnt++)
  {
    #if (ETH_CTRL_ENABLE_RX_POLLING == STD_ON)
    ...
    GusRxFrameCnt[LucCtrlCnt] = 0;
    GusMsgLength[LucCtrlCnt] = 0;
  }
  ...
  for (LucCtrlCnt = 0; LucCtrlCnt < ETH_TOTAL_CTRL_CONFIG; LucCtrlCnt++)
  {
    ...
    if (memcmp(&LucRetrieveMacAddr[LucCtrlCnt][0], &mac_board_addr[LucCtrlCnt][0], SRC_MACADDR_LEN) == 0)
    {
    ...
    Eth_UpdateStreamFilter(LucCtrlCnt, RX_QUEUE_2, streamId0);
    #endif

    #if (ETH_UPDATE_PHYS_ADDR_FILTER == STD_ON)
    /* Add addition multicast address for this controller to listen to */
    Eth_UpdatePhysAddrFilter(LucCtrlCnt, mac_multi_addr0, ETH_ADD_TO_FILTER);
  ...
  for (LucCtrlCnt = 0; LucCtrlCnt < ETH_TOTAL_CTRL_CONFIG; LucCtrlCnt++)
  {
    ...
    SendData(LucCtrlCnt, &TxEthFrame[0][0], FRAME_LEN, FRAME_LEN, 0, mac_tgt_addr);
    ...
    #if (ETH_MACRO_ETNE == STD_ON)
    GulPortId = ((Eth_ETNEConfigType*)Eth_GpCtrlConfigPtr[LucCtrlCnt].pHwUnitConfig)->ulEthPortId;
    if (ETH_RSW2_PORT_TYPE_GWCA == Eth_GaaRsw2PortType[GulPortId])
    {
      /* Assign Controller Index to Temperature varriable */
      LucCtrlTemp = LucCtrlCnt;
      /* Forwarding reception frame to ETHAx */
      LucCtrlCnt = ETHRSW0;
    ...
    if (memcmp((void *)&GaaRxEthFrame[LucCtrlCnt][0], &TxEthFrame[GusRxFrameCnt[LucCtrlCnt] - 1][0],
        GusMsgLength[LucCtrlCnt] - ETH_NORMALMAC_HSIZE) == 0)
    {
      EthPassedCount[EthCheckCount] = ETH_PASSED;
    }
    EthCheckCount++;
    ...
    SendData(LucCtrlCnt, &TxEthFrame[4][0], FRAME_LEN, FRAME_LEN, 0, mac_multi_addr0);

    /* Send multicast frame which is not valid for this station, it will be filtered out */
    SendData(LucCtrlCnt, &TxEthFrame[4][0], FRAME_LEN, FRAME_LEN, 0, mac_multi_addr1);

    /* Send Network control frame */
    SendData(LucCtrlCnt, &TxEthFrame[1][0], FRAME_LEN, ETH_NC_TYPE, 0, mac_nc_addr);

    /* Send 1722 frames with no reserved resource in this station */
    SendData(LucCtrlCnt, &TxEthFrame[5][0], FRAME_LEN, ETH_AVBTP_TYPE, 0, mac_tgt_addr0);

    /* Send 1722 frames with reserved resource in this station */
    SendData(LucCtrlCnt, &TxEthFrame[2][0], FRAME_LEN, ETH_AVBTP_TYPE, 0, mac_tgt_addr0);
    SendData(LucCtrlCnt, &TxEthFrame[3][0], FRAME_LEN, ETH_AVBTP_TYPE, 0, mac_tgt_addr1);

    #if (ETH_CTRL_ENABLE_RX_POLLING == STD_ON)
    while (GusRxFrameCnt[LucCtrlCnt] != EthTxTotal[LucCtrlCnt])
    {
      Eth_MainFunction();
      ...
      do
      {
        ...
        Eth_Receive(LucCtrlCnt, RX_QUEUE_3, &LenEthStatusRxQueue3[LucCtrlCnt]);
            ...
            #endif
            && (GusRxFrameCnt[LucCtrlCnt] != EthTxTotal[LucCtrlCnt]));
      #else
      do
      {
        /* Check for received message */
  ...
  for (LulInc = 0; LulInc < EthCheckCount; LulInc++)
  {
    if (EthPassedCount[LulInc] != ETH_PASSED)
    {
      sample_NG_end();
    }
  }
...
)
{
  ...
  do
  {
    ...
    Eth_TxConfirmation(CtrlIdx);
  } while (0 == GucTxConfirmed[CtrlIdx]);
  #endif
}
