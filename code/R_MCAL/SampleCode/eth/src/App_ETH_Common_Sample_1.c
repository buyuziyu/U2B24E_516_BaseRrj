...
/* Module       = App_ETH_Common_Sample_1.c                                   */
/*============================================================================*/
/*                                  COPYRIGHT                                 */
/*============================================================================*/
...
Eth_TimeStampType TxTimeStamp_1[ETH_TOTAL_CTRL_CONFIG];
#endif

/* Board MAC address */
static uint8 mac_board_addr_1[2][SRC_MACADDR_LEN] =
{
...
/* Target MAC address */
static uint8 mac_tgt_addr_1[DST_MACADDR_LEN] = {0x74U, 0x90U, 0x50U, 0xFFU, 0xFFU, 0xFFU};

/* IEEE 1722 frames MAC address*/
static uint8 mac_tgt_addr_10[DST_MACADDR_LEN] = {0x91U, 0xE0U, 0xF0U, 0x00U, 0x00U, 0x00U};
static uint8 mac_tgt_addr_11[DST_MACADDR_LEN] = {0x91U, 0xE0U, 0xF0U, 0x00U, 0xFEU, 0xFFU};

/* Network control MAC address */
static uint8 mac_nc_addr_1[DST_MACADDR_LEN] = {0x01U, 0x80U, 0xC2U, 0x00U, 0x00U, 0x0EU};

/* Valid multicast MAC address used for Eth_UpdatePhysAddrFilter */
static uint8 mac_multi_addr0_1[DST_MACADDR_LEN] =  {0x11U, 0x22U, 0x33U, 0x44U, 0x55U, 0x66U};
/* Invalid multicast MAC address used for Eth_UpdatePhysAddrFilter */
static uint8 mac_multi_addr1_1[DST_MACADDR_LEN] = {0x11U, 0x22U, 0x33U, 0x44U, 0x55U, 0x77U};

#if (ETH_STREAM_FILTERING == STD_ON)
static uint8 streamId0_1[STREAMID_LEN] = {0x11U, 0x22U, 0x33U, 0x44U, 0x55U, 0xFFU, 0x00U, 0x00U};
#endif

/*---------------------------------------------------*/
/* Receive and Transmit Buffers */
/*---------------------------------------------------*/
static uint8 TxEthFrame_1[SAMPLE_TOTAL][FRAME_LEN] =
{
...
int main_PE1(void)
{
  ...
  LucCoreIndex = Eth_Config->pCoreId2Index[CORE_1];
  LulTotalCtrlConfig = Eth_Config->pTotalCtrlConfig[LucCoreIndex];

  for (LucCount = 0; LucCount < LulTotalCtrlConfig; LucCount++)
  {
    LucCtrlCnt = Eth_Config->ulCtrlConfigIdx[CORE_1][LucCount];
    #if (ETH_CTRL_ENABLE_RX_POLLING == STD_ON)
    LenEthCtrlMode[LucCtrlCnt] = ETH_MODE_DOWN;
    LenEthRxStatus[LucCtrlCnt] = ETH_NOT_RECEIVED;
    #endif

    GusRxFrameCnt_1[LucCtrlCnt] = 0;
    GusMsgLength_1[LucCtrlCnt] = 0;
  }
  ...
  /* Wait core 0 complete init stub */
  while (GblInitStatus[0] != 1);
  /************************** Initilazation  *****************************/
  ...
  for (LucCount = 0; LucCount < LulTotalCtrlConfig; LucCount++)
  {
    ...
    Eth_SetPhysAddr(LucCtrlCnt, mac_board_addr_1[LucCtrlCnt]);

    /* Invoke Eth_GetPhysAddr to get physical address */
    Eth_GetPhysAddr(LucCtrlCnt, (uint8*)LucRetrieveMacAddr[LucCtrlCnt]);

    if (memcmp(&LucRetrieveMacAddr[LucCtrlCnt][0], &mac_board_addr_1[LucCtrlCnt][0], SRC_MACADDR_LEN) == 0)
    {
    ...
    Eth_UpdateStreamFilter(LucCtrlCnt, RX_QUEUE_2, streamId0_1);
    #endif

    #if (ETH_UPDATE_PHYS_ADDR_FILTER == STD_ON)
    /* Add addition multicast address for this controller to listen to */
    Eth_UpdatePhysAddrFilter(LucCtrlCnt, mac_multi_addr0_1, ETH_ADD_TO_FILTER);

    /* Add addition multicast address for this controller to listen to */
    Eth_UpdatePhysAddrFilter(LucCtrlCnt, mac_tgt_addr_10, ETH_ADD_TO_FILTER);
    #endif
  ...
  for (LucCount = 0; LucCount < LulTotalCtrlConfig; LucCount++)
  {
    ...
    SendData(LucCtrlCnt, &TxEthFrame_1[0][0], FRAME_LEN, FRAME_LEN, 0, mac_tgt_addr_1);
    ...
    if ((ETH_VALID == LenEthTimeQualPtr) &&
        ((LstCurrentTime.secondsHi > TxTimeStamp_1[LucCtrlCnt].secondsHi) ||
        ((LstCurrentTime.secondsHi == TxTimeStamp_1[LucCtrlCnt].secondsHi) &&
         (LstCurrentTime.seconds > TxTimeStamp_1[LucCtrlCnt].seconds)) ||
        ((LstCurrentTime.secondsHi == TxTimeStamp_1[LucCtrlCnt].secondsHi) &&
         (LstCurrentTime.seconds == TxTimeStamp_1[LucCtrlCnt].seconds) &&
        (LstCurrentTime.nanoseconds > TxTimeStamp_1[LucCtrlCnt].nanoseconds))))
    {
      EthPassedCount_1[EthCheckCount_1] = ETH_PASSED;
    }
    EthCheckCount_1++;
    ...
    if (memcmp((void *)&GaaRxEthFrame_1[LucCtrlCnt][0], &TxEthFrame_1[GusRxFrameCnt_1[LucCtrlCnt] - 1][0],
        GusMsgLength_1[LucCtrlCnt] - ETH_NORMALMAC_HSIZE_1) == 0)
    {
      EthPassedCount_1[EthCheckCount_1] = ETH_PASSED;
    }
    EthCheckCount_1++;
    ...
    SendData(LucCtrlCnt, &TxEthFrame_1[4][0], FRAME_LEN, FRAME_LEN, 0, mac_multi_addr0_1);

    /* Send multicast frame which is not valid for this station, it will be filtered out */
    SendData(LucCtrlCnt, &TxEthFrame_1[4][0], FRAME_LEN, FRAME_LEN, 0, mac_multi_addr1_1);

    /* Send Network control frame */
    SendData(LucCtrlCnt, &TxEthFrame_1[1][0], FRAME_LEN, ETH_NC_TYPE, 0, mac_nc_addr_1);

    /* Send 1722 frames with no reserved resource in this station */
    SendData(LucCtrlCnt, &TxEthFrame_1[5][0], FRAME_LEN, ETH_AVBTP_TYPE, 0, mac_tgt_addr_10);

    /* Send 1722 frames with reserved resource in this station */
    SendData(LucCtrlCnt, &TxEthFrame_1[2][0], FRAME_LEN, ETH_AVBTP_TYPE, 0, mac_tgt_addr_10);
    SendData(LucCtrlCnt, &TxEthFrame_1[3][0], FRAME_LEN, ETH_AVBTP_TYPE, 0, mac_tgt_addr_11);

    #if (ETH_CTRL_ENABLE_RX_POLLING == STD_ON)
    while (GusRxFrameCnt_1[LucCtrlCnt] != EthTxTotal[LucCtrlCnt])
    {
      Eth_MainFunction();
      ...
      do
      {
        ...
        Eth_Receive(LucCtrlCnt, RX_QUEUE_3, &LenEthStatusRxQueue3[LucCtrlCnt]);
            ...
            #endif
            && (GusRxFrameCnt_1[LucCtrlCnt] != EthTxTotal[LucCtrlCnt]));
      #else
      do
      {
        /* Check for received message */
  ...
  for (LulInc = 0; LulInc < EthCheckCount_1; LulInc++)
  {
    if (EthPassedCount_1[LulInc] != ETH_PASSED)
    {
      sample_NG_end();
    }
  }
...
)
{
  ...
  do
  {
    ...
    Eth_TxConfirmation(CtrlIdx);
  } while (0 == GucTxConfirmed_1[CtrlIdx]);
  #endif
}
