...
static bool add_detecting_overflow(time_t *accumulator, time_t change)
    /* Return true if overflow, false otherwise. */
{
    if     ((*accumulator > 0 && change > __GHS_TIME_T_MAX - *accumulator) ||
	    (*accumulator < 0 && change < __GHS_TIME_T_MIN - *accumulator)) {
...
struct tm *localtime_r(const time_t *timer, struct tm *result)
{
...
#if defined(__INTEGRITY_SHARED_LIBS)
    tzset();
    overflow = add_detecting_overflow(&time_v, __ghs_timezone);
#else
    overflow = add_detecting_overflow(&time_v, - 60 * __gh_timezone());
#endif

    /* The timezone adjustment above can overflow. In that case, return NULL
     * rather than a wrapped struct tm. */
    if (overflow)
	return NULL;
    ...
    if (dst_offset != 0) {
	/* Check if the DST adjustment overflowed. */
	overflow = add_detecting_overflow(&time_v, dst_offset);
	if (overflow)
	    return NULL;
