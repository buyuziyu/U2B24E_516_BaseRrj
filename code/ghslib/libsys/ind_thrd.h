...
enum __ghs_ThreadLocalStorage_specifier {
    ...
    __ghs_TLS_malloc_arena_index,
    __ghs_TLS_malloc_arena_pointer
};
...
#define __ghs_SafeGetThreadLocalStorageItem(item) \
    (!__ghs_weak_func_exists(__ghs_GetThreadLocalStorageItem) ? \
...
#define __ghs_ProfileLock() { 						\
    if (__ghs_weak_func_exists(__ghs_InterruptLock))   			\
	__ghs_InterruptLock();						\
    else								\
	__ghsLock();							\
}
#define __ghs_ProfileUnlock() { 					\
    if (__ghs_weak_func_exists(__ghs_InterruptUnlock))			\
...
#define __ghs_CurrentThreadID() \
    (__ghs_weak_func_exists(__ghs_GetCurrentThreadID) ? __ghs_GetCurrentThreadID() : 0)
